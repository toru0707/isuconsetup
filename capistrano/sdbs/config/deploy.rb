...
namespace :deploy do
  ...
  task :seed do
	run "cd #{current_path} && bundle exec rake db:fixtures:load RAILS_ENV=#{rails_env}"
	run "mysql -u root -ppassword sdbs_development < #{current_path}/test/fixtures/sql/sdbs.sql"
  end
end
